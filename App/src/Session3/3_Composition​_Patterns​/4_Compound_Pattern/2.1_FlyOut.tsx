  ...
  return (
    <div>
      {React.Children.map(props.children, (child) => {
        return React.isValidElement(child) ? React.cloneElement(child, { open, toggle, value, setValue }) : child
      }
      )}
    </div>
  );
...
const Input: React.FC<InputProps> = ({ value = "", toggle, setValue, ...props }) => {
  return <input onFocus={toggle} onBlur={toggle} value={value} onChange={(e) => setValue(e.target.value)} {...props} />;
};
  ...
  return open ?
    <ul>
      {React.Children.map(children, (child) =>
        React.isValidElement(child) ? React.cloneElement(child, { setValue }) : child
      )}
    </ul> :
    null;
};
